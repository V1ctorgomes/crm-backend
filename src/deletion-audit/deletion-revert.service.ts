...
export class DeletionRevertService {
  ...
  async revertUserDeletion(auditId: string, adminUserId: string) {
    ...
    try {
      if (row.resourceType === DeletionResourceType.INSTANCE) {
        await this.revertInstanceWithEvolution(row.snapshot, auditId, adminUserId);
        return { success: true };
      }
   ...
   */
  private async revertInstanceWithEvolution(
    snapshot: Prisma.JsonValue,
    auditId: string,
    adminUserId: string,
  ): Promise<void> {
    ...
    const evolutionErrorMessage = (error: unknown): string => {
      const err = error as { response?: { data?: { message?: unknown; error?: string } }; message?: string };
      const resData = err?.response?.data;
      if (resData?.message) {
        if (Array.isArray(resData.message)) {
          return resData.message.map((m: { message?: string } | string) =>
    ...
    try {
      ...
      if (proxyHost && proxyPort) {
        ...
        try {
          ...
          throw new HttpException(
            `A Evolution rejeitou o proxy ao restaurar: ${evolutionErrorMessage(proxyErr)}`,
            HttpStatus.BAD_REQUEST,
          );
        }
      }
      ...
      try {
        ...
        this.logger.warn(`settings/set ao restaurar ${name}: ${evolutionErrorMessage(e)}`);
      }
      ...
      throw new HttpException(evolutionErrorMessage(e), HttpStatus.BAD_REQUEST);
    }
  }
}
